...
export const SideBarMenuItem = ({ item }: { item: SideNavItem }) => {
  ...
  const navMenuDropdownItem = "text-white py-2 px-4 hover:text-blue-600 transition duration-200";
            ...
            <div className="grid gap-y-2 px-10 py-3 leading-5">
              {
                item.subMenuItems.map((subItem, index)=>{
                  return(
                    <Link key={index} href={subItem.path} className={navMenuDropdownItem}>
                      <span>
                        {subItem.title}
                      </span>
                    </Link>
