...
const AboutStatistics: React.FC = () => {
  useEffect(() => {
    ...
    const dataLengthAgainstAge = [
      ...
      45, 46, 48, 50, 51.5, 52, 54, 55.5, 57, 58.5, 60, 62, 
      // Second year (12-23 months)
      65, 66.5, 67.5, 69, 70.5, 72, 73.5, 75, 76, 78, 80, 82, 
      // Third year (24-35 months)
      85, 87.5, 88, 90, 100, 102.5, 103, 105, 106, 107, 108, 109, 
      // Fourth year (36-47 months)
    110, 110.5, 111.5, 112.5, 113.5, 114.5, 115, 115.5, 116, 116.5, 117, 117.5, 
      // Fifth year (48-59 months)
      118, 118.5, 119, 119.5, 120.5, 121, 122, 123, 124, 124.5, 125, 125, 
      // Sixth year (60 months)
      125 // The child's height will reach 125 cm by the end of the sixth year
    ];     

    const labelsMonths = Array.from({ length: 60 }, (_, i) => (i + 1).toString()); // Convert numbers to strings
        ...
        scales: {
          ...
          y: {
            ...
            ticks: {
              callback: (value: string | number, index: number, values: Tick[]) => {
                const typedValues = values as unknown as (string | number)[];
                if (typeof value === 'number' && value >= 45 && value % 5 === 0) {
                  return value.toString();
                }
                return '';
              }
  ...
  return (
    <div className="flex flex-col w-full 2xl:w-2/3 min-w-full ">
      <div className="flex-1 bg-white rounded-lg shadow-xl mt-4 p-8 border border-rchs">
        <h2 className="text-xl text-gray-900 font-bold text-center">Child Nutritional Status (Height Vs Age)</h2>
